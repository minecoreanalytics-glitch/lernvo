...
function mobileHandler(handler: (req: Request, res: Response) => Promise<unknown>) {
...
router.get('/bootstrap', authenticate, mobileHandler(async (req, res) => {
...
router.get('/me', authenticate, mobileHandler(async (req, res) => {
...
router.get('/inbox', authenticate, mobileHandler(async (req, res) => {
  const inbox = await loadInbox(req.user!.userId);
  return sendEnvelope(req, res, inbox);
}));

router.post('/inbox/read', authenticate, mobileHandler(async (req, res) => {
...
router.get('/leaderboard', authenticate, mobileHandler(async (req, res) => {
...
router.get('/media-token', authenticate, mobileHandler(async (req, res) => {
...
router.get('/team', authenticate, mobileHandler(async (req, res) => {
...
router.post('/ask', authenticate, mobileHandler(async (req, res) => {
...
router.get('/modules/:id', authenticate, mobileHandler(async (req, res) => {
...
router.post('/modules/:id/start', authenticate, mobileHandler(async (req, res) => {
...
router.post('/contents/:id/progress', authenticate, mobileHandler(async (req, res) => {
...
router.get('/quizzes/:id', authenticate, mobileHandler(async (req, res) => {
...
router.post('/quizzes/:id/submit', authenticate, mobileHandler(async (req, res) => {
  ...
  if (submitted.error === 'INVALID_ANSWERS') {
    return sendError(res, 400, 'INVALID_ANSWERS', 'Answer each quiz question exactly once using a valid option');
  }
  if (submitted.error === 'CONTENT_INCOMPLETE') {
    return sendError(res, 403, 'CONTENT_INCOMPLETE', 'Complete the required sections before taking this quiz');
  }
  if (submitted.error === 'MAX_ATTEMPTS') {
    return sendError(res, 403, 'MAX_ATTEMPTS', 'The attempt limit has been reached');
  }
  return sendEnvelope(req, res, submitted.result);
}));

router.post('/sync/events', authenticate, mobileHandler(async (req, res) => {
...
router.use((error: unknown, req: Request, res: Response, next: NextFunction) => {
  ...
  return sendError(res, 500, 'INTERNAL_ERROR', 'Lernvo is temporarily unavailable. Try again shortly.');
});
