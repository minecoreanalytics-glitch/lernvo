...
describe('login tenant gate', () => {
  it('refuses login when tenant is PENDING', async () => {
    ...
    await tenantStore.run({ tenantId: null, superAdmin: true }, async () => {
      ...
      await prisma.user.create({ data: { email: e, passwordHash: await bcrypt.hash('StrongPass123!', 12), firstName: 'a', lastName: 'b', role: 'PLATFORM_MANAGER', isActive: true, tenantId: t.id } })
    })
